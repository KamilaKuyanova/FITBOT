...
export default function GlobalStylistScreen() {
  ...
  // Try to auto-fill weather from profile if available
  useEffect(() => {
    // Weather would come from HomeScreen's weather data, but for now we'll let user input it
    // In a real app, you'd pass weather data via navigation params or context
  }, []);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/ai/global-outfit`, {
        method: 'POST',
        headers: {
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      <View style={styles.header}>
        <Text style={styles.title}>✨ Global AI Stylist</Text>
        <Text style={styles.subtitle}>Generate outfits from AI knowledge</Text>
      </View>
      ...
      >
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate("ClosetStylist")}
          activeOpacity={0.7}
        >
          <Text style={styles.linkText}>
            Prefer to use your own clothes? → Try 'Outfits from my wardrobe'
          </Text>
        </TouchableOpacity>
