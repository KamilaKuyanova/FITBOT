...
import { AI_CHAT_API_URL } from "../config/api";
...
export function AIChatProvider({ children }: { children?: ReactNode }) {
  ...
  const sendMessage = async (userMessage: string, closetData?: any): Promise<void> => {
    console.log("[UI] AI chat message sent:", userMessage);
    // For now, use mock responses. Can be enabled via config later
    ...
    try {
      ...
      if (useBackend && closetData) {
        // Use backend API
        console.log("[UI] Sending message to AI API:", `${AI_CHAT_API_URL}/api/ai/chat`);
        const response = await fetch(`${AI_CHAT_API_URL}/api/ai/chat`, {
          method: 'POST',
          headers: {
        ...
        console.log("[UI] AI chat API response status:", response.status);
        if (!response.ok) {
          const errorData = await response.json();
          console.error("[UI] AI chat API response error:", errorData);
          throw new Error('Failed to get AI response');
        }

        const data = await response.json();
        console.log("[UI] AI chat response received:", data);
