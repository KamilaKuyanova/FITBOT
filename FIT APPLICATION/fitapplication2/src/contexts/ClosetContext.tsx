import React, {
  ...
  function loadInitialItems(): ClothingItem[] {
    ...
    try {
      ...
      console.error("Failed to load wardrobeItems from localStorage:", error);
    }
  ...
  export function ClosetProvider({
    ...
    const [items, setItems] = useState<ClothingItem[]>(() => loadInitialItems());
  
    // универсальное сохранение в localStorage при изменении items
    useEffect(() => {
      try {
        ...
        console.error("Failed to save wardrobeItems to localStorage:", error);
      }
    }, [items]);
